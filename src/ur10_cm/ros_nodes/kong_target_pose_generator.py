...
from kong_ur10 import robot_control
...
if __name__ == '__main__':
    rospy.init_node("kong_target_pose_generator", anonymous=True)

    group = "kong_arm"
    topic = "/kong_ur10/pose_to_follow"

    kong_ur10 = robot_control(group) #Intialize robot control class

    kong_pose_publisher = pose_generator(topic)

    start_pose = kong_ur10._group.get_current_pose()
    ...
    kong_pose_publisher.interpolate_two_poses(start_pose, end_pose, 2) #Stores interpolation in self._waypoints

    kong_pose_publisher.publish_target_pose(loop=True)

...
%{
This scripts outputs the positive and negative vector fields. The vector fields
(over an annulus) correspond to the instantaneous heading direction of the base
of an oblique cone whose apex is fixed.
%}


%Set parameter values
R = sym(0.35); % Base radius of the cone
hc = sym(1.5); %sym(1.35*sind(90)); % Vertical height of the cone
r = sym(0.35 + 1.5*cosd(90)); % Oblique cone's eccentricity. Horizontal distance between 
h = sym(1.35); %sym(1.26); % Height of the cone's apex (assumed fixed) above ground 
...
for del_query = linspace(del_lb, del_ub-0.005, radial_res)
...
quiver(X_ann_pos, Y_ann_pos, U_ann_pos, V_ann_pos,...
     0.5, 'LineWidth', 1.0, 'Color', [.4 .4 .4])
hold on

function vector_fields(cone_radius, cone_length, cone_eccentricity, apex_height, matlab_dir_path)
  ...
  arguments:
   ...
   cone_length: height of cone
   cone_eccentricity: apex of cone offset relative to its base center
   apex_height: height of apex about ground
  %}


  %Set parameter values
  R = sym(cone_radius); % Base radius of the cone
  hc = sym(cone_length); % Vertical height of the cone
  r = sym(cone_eccentricity); % Oblique cone's eccentricity
  h = sym(apex_height); % Height of the cone's apex (assumed fixed) above ground
